...
const Cms = ({ setCms }) => {
    ...
    console.log('hdjd', errMsg)

    return (
        <div className='cms-container'>
            <h1 onClick={() => setCms(false)} style={{
                color: '#FFF', top: '20px', right: '20px', fontSize: '24px', fontWeight: '700',
            ...
            <div className='edit-container'>
                ...
                <div className='text-section'>
                    ...
                    {errMsg.length !== 0 && (
                        <p style={{ color: 'red', fontSize: '15px', position: 'absolute' }}>
                            {errMsg}
                        </p>
                    )}
                    <div className='btn-container'>
                        <button className='buttons btn' onClick={() => setShowEdit(true)}>Edit</button>
                        <button onClick={addHeadline} disabled={updateHeadline.length === 0} className='buttons btn'>Save</button>
                    </div>
                </div>
            </div>
        </div>
